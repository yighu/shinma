...
local function onSceneTouch( self, event )
	if event.phase == "began" then
		...
		prevpage="prayscript32"
		composer.gotoScene( nextpage, "slideLeft", 800  )
		else
		nextpage="prayscript32"
...
function scene:show( event )
	...
	local sceneGroup = self.view
	text1 = display.newText( a[3][2], 0, 0, native.systemFontBold, 24 )
	text1:setFillColor( text_R,text_G,text_B)
	text1.x, text1.y = display.contentWidth * ratiot, titley0
	sceneGroup:insert( text1 )
	text2 = display.newText( a[3][3], 0, 0, display.contentWidth*ratiow,display.contentHeight*ratiow,native.systemFont, fontsize)
